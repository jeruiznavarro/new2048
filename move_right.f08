subroutine move_right(auxiliary_empty_cells,auxiliary_empty_cells_history,current_empty_cells_history,empty_cells,empty_cells_history,matrix,matrix_history,score_history)
	...
	do j=0,cells-1
		...
		do i=cells-1,1,-1
			if(k<i)cycle!moving on to the next non null cell in case that there are several non null cells one after the other
			if((matrix(i,j)==matrix(i-1,j)).and.(matrix(i,j)>0))then
				no_cells_moved=.false.!at least one cell has moved
				tag=coordinates_to_index(i-1,j,cells)!storing the index to avoid calling the function more than once
				matrix(i,j)=matrix(i,j)+matrix(i-1,j)!if two adjacent cells have the same value their become one with the sum of their previous values
				...
				matrix(i-1,j)=0!and the cell that was added to the previous one is erased
				...
				k=i-1!setting the first auxiliary counter to find the next non null cell
				if(i<cells-1)then
					l=i+1!setting the second auxiliary counter in case there are previous non null cells
				else
					l=i!avoid dangerous indeces
				end if
				exit_1=.false.!resetting this variable for the recursive search below
				exit_2=.false.!same as above
				do
					if((l<cells-1).and.(matrix(l,j)==0))then
				...
				if(matrix(l,j)==matrix(k,j))then
					no_cells_moved=.false.!at least one cell has moved
					tag=coordinates_to_index(k,j,cells)!storing the index to avoid calling the function more than once
					matrix(l,j)=matrix(l,j)+matrix(k,j)!if two adjacent cells have the same value their become one with the sum of their previous values
					if(matrix(l,j)>262144)then
						game_over=.true.!maximum cell value achieved, this game has been won
						return!going back to the main program
					end if
					score=score+matrix(l,j)!updating the score
					matrix(k,j)=0!and the cell that was added to the previous one is erased
					current_empty_cells=current_empty_cells+1!one more empty cell
					empty_cells(current_empty_cells-1)=tag!recording the new empty cell
					auxiliary_empty_cells(tag)=current_empty_cells-1!updating the auxiliary empty cells
				else
					no_cells_moved=.false.!at least one cell has moved
					if(matrix(l,j)==0)then
						tag=coordinates_to_index(l,j,cells)!storing the index of the empty cell to avoid calling the function more than once
						matrix(l,j)=matrix(k,j)!this cell is moved to the spot held by the empty cell with coordinates (i,j)
					else
						tag=coordinates_to_index(l-1,j,cells)!storing the index of the empty cell to avoid calling the function more than once
						matrix(l-1,j)=matrix(k,j)!this cell is moved to the spot held by the empty cell with coordinates (i,j)
					end if
					tag_extra=coordinates_to_index(k,j,cells)!storing the index of the occupied cell to avoid calling the function more than once
					matrix(k,j)=0!and the cell that was added to the previous one is erased
